...
def main():
    ...
    np.savetxt(os.path.join(args.output, "mag_nodes.txt"), 
               node_data, fmt="%d", delimiter=" ")
    ...
    dgl.data.utils.save_tensors(os.path.join(args.output, "node_feat.dgl"), node_feats)
    ...
    np.savetxt(os.path.join(args.output, "mag_edges.txt"), 
               edge_data.numpy(), fmt="%d", delimiter=" ")
    ...
    dgl.data.utils.save_tensors(os.path.join(args.output, "edge_feat.dgl"), edge_feats)
    
    # Store the basic metadata of the graph.
    graph_stats = [g.num_nodes(), len(src_id), num_node_weights]
    with open(os.path.join(args.output, "mag_stats.txt"), "w") as filehandle:
    ...
    with open(os.path.join(args.output, "mag.json"), "w") as outfile:
